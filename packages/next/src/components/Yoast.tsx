		...
		const options: HTMLReactParserOptions = {
			...
			replace: (domNode) => {
				if (isElement(domNode)) {
					...
					// html-react-parser@6 types attributesToProps as returning
					// `string | boolean` values (boolean for valueless attributes like
					// `async`). The URL props below are always strings in Yoast head
					// markup, but the types no longer say so.
