...
use WP_HTML_Decoder;
use WP_HTML_Doctype_Info;
use WP_HTML_Span;
use WP_HTML_Text_Replacement;

/**
 * Custom HTML Tag Processor that preserves character references in attributes.
 *
 * Copied from https://github.com/WordPress/wordpress-develop/blob/dfe773a22d594db9431508133e78b5c3df027a31/src/wp-includes/html-api/class-wp-html-tag-processor.php
 *
 * This class is a copy of WP_HTML_Tag_Processor but using htmlspecialchars() instead
 * of esc_attr() when setting attribute values, so that set_attribute_fixed encodes arbitrary
 * strings to HTML rather than mangling them
 * set_attribute_fixed('data-wp-block', '&#39;') creates an attribute whose decoded value is &#39;
 * whereas set_attribute('data-wp-block', '&#39;') decodes it to '.
 */
class Fixed_WP_HTML_Tag_Processor {
	/**
	...
	/**
	 * Sets value of attribute with the given name to the given value.
	 * 
	 * This is identical to the set_attribute() method except it uses
	 * htmlspecialchars() instead of esc_attr() so that attributes are not lossy
	 ...
	 * @return bool Whether an attribute value was set.
	 */
	public function set_attribute_fixed( $name, $value ): bool {
		if (
			self::STATE_MATCHED_TAG !== $this->parser_state ||
			$this->is_closing_tag
		) {
		...
		if ( true === $value ) {
			...
			/*
			 * Escape URL attributes using esc_url, but for all other attributes
			 * use htmlspecialchars instead of esc_attr to preserve character references.
			 *
			 * @see https://html.spec.whatwg.org/#attributes-3
			 */
			if ( in_array( $comparable_name, wp_kses_uri_attributes(), true ) ) {
				$escaped_new_value = esc_url( $value );
			} else {
				// Use htmlspecialchars instead of esc_attr to preserve character references
				$escaped_new_value = htmlspecialchars( $value, ENT_QUOTES, 'UTF-8', true );
			}
